...
void enterInfo()
{
    ...
    printf("\nChoose the day of the week | only integer value allowed\n\n");
    int choice;
    ...
    printf("7. Sunday\n\n");
    scanf("%d",&choice);
    //takes in any left over values from the functions that might disturb the system
    char leftover;
    scanf("%c", &leftover);
    //control gate for each of the days of the week the user has suggested
    switch(choice)
    {
        case (1) :
            Monday();
...
void reset()
{
    //writes blank over all the files??????
    FILE * Mon;
...
void control()      //the gateway from the login to 3 choices
{
    ...
    int continuing;//comment out
    int check;//comment out
...
void count(){
    ...
    int counter[SIZE]= {1,2,3,4,5,6,7};
    //counter[0] = 0;
    //int i = 0;
    //int j = 0;
    //i++;
    ...
    printf("Number of times logged off: %d\n\n\n\n",*count);
